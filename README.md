...
长文档左侧自动生成大纲，一键跳到任意章节；明暗两套主题跟随系统，晚上读不刺眼；需要留档时一键导出 PDF。
...
双击任意段落、标题、列表或表格，它就地变成编辑框；改完 Ctrl+Enter，立刻变回排版好的样子。不用切换"编辑模式 / 预览模式"，不用盯着源代码，读到哪、改到哪。
...
- **实时编辑（Obsidian 式）**：双击块就地编辑、`Ctrl+Enter` 提交、`Esc` 放弃、清空即删除该块；块与源码的映射靠渲染期 `data-line` 行号锚（VS Code 预览同款机制），浏览器对原始 HTML 的重排嵌套不会破坏定位；含原始 HTML 的折叠块（`<details>` 等）支持整段编辑
