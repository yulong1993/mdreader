...
- **实时编辑（Obsidian 式）**：双击块就地编辑，编辑排版继承所在块（标题保持大号粗体），Markdown 标记（`##`、`**`、表格竖线等）灰色淡化显示（透明 textarea + 同度量高亮背板，输入法合成文本可见）；光标/选区经"渲染字符 ↔ 源码字符"映射落在双击处；`Ctrl+Enter` 提交、`Esc` 放弃、清空即删除该块。块与源码的映射靠渲染期 `data-line` 行号锚（VS Code 预览同款机制），浏览器对原始 HTML 的重排嵌套不会破坏定位；含原始 HTML 的折叠块（`<details>` 等）支持整段编辑
