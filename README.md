...
- **编辑模式**：`Ctrl+E` 分屏编辑（左源码右实时预览，400ms 防抖刷新），`Ctrl+S` 保存（原子写入：临时文件+替换，白名单扩展名）；未保存有「●」提示；关闭窗口时弹**应用内**三按钮确认（保存并退出 / 直接退出 / 取消，Esc=取消 Enter=保存；保存失败则留在窗口不退出，内容不丢）；切换文档时自动保存（Obsidian 式，保存失败不切换）；编辑期间外部修改不覆盖未保存的编辑
