...
- **Obsidian 双链**：`[[页面]]`、`[[页面|别名]]`、`[[页面#标题]]`（跳转后滚动定位）；`![[嵌入]]` 支持图片/音频/视频与 `|200`、`|100x200` 尺寸语法（md 嵌入降级为链接卡片）。链接与图片目标先按相对路径解析，找不到再在当前目录及子目录（≤3 层）按文件名递归查找——`![[图.png]]` 不带路径也能找到 attachments/ 里的图；仍找不到显示灰色缺失占位。支持中文与空格文件名
...
- **实时编辑（Obsidian 式）**：**双击任意段落/标题/列表/表格**直接开始编辑该块（无需先按快捷键；或 `Ctrl+E` 进入编辑模式后单击块），该块就地变成源码编辑框（与 Obsidian 实时预览一致：正在编辑的块显示源码，其余保持渲染）；点别处或 `Ctrl+Enter` 提交即渲染，点空白处提交并退出编辑模式，`Esc` 放弃，清空整块内容即删除该块；`Ctrl+S` 保存（原子写入：临时文件+替换，白名单扩展名）；未保存有「●」提示；关闭窗口时弹**应用内**三按钮确认（保存并退出 / 直接退出 / 取消，Esc=取消 Enter=保存；保存失败则留在窗口不退出，内容不丢）；切换文档时自动保存（Obsidian 式，保存失败不切换）；编辑期间外部修改不覆盖未保存的编辑。脚注定义行、链接引用定义行等无法对应源码行的内容暂不支持块编辑；个别渲染结构无法与源码对账的文档（如含被消毒剥离标签的原始 HTML）会自动禁用块编辑以防改错位置
...
- **编码**：UTF-8（含 BOM）自动识别，GBK 老文档自动回退解码；保存时统一写为 UTF-8（无 BOM），UTF-16/GBK 文档保存后编码转为 UTF-8（内容不变）
- **打开失败有提示**：文件超限、编码损坏或被锁定时弹窗说明原因（启动参数 / 双击关联 / Ctrl+O 三个入口统一）
...
- **直接用 cargo 构建绿色版必须带 feature**：`cargo build --release --features custom-protocol`。缺了它产出的是连 `localhost:1420` 的开发版二进制（页面显示"无法访问此页面"）；`npm run tauri build` 不受影响，CLI 会自动带上。
