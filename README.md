...
- **实时编辑（Obsidian 式）**：`Ctrl+E` 进入编辑模式后，**点击任意段落/标题/列表/表格**，该块就地变成源码编辑框（与 Obsidian 实时预览一致：正在编辑的块显示源码，其余保持渲染）；点别处或 `Ctrl+Enter` 提交即渲染，`Esc` 放弃，清空整块内容即删除该块；`Ctrl+S` 保存（原子写入：临时文件+替换，白名单扩展名）；未保存有「●」提示；关闭窗口时弹**应用内**三按钮确认（保存并退出 / 直接退出 / 取消，Esc=取消 Enter=保存；保存失败则留在窗口不退出，内容不丢）；切换文档时自动保存（Obsidian 式，保存失败不切换）；编辑期间外部修改不覆盖未保存的编辑
